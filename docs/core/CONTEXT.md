...
**Delivery Work Context**:
A transient, never-stored context Gorchestra prepares around Delivery work. The stored phase contains root-level validated Portfolio facts and derived Delivery Work State for one Delivery. The runtime phase extends the stored phase for one Delivery work pass with resolved Delivery Config, selected execution Model, and provider access values needed by Core-owned runtime and provider behavior.
