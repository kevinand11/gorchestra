...
Model reasoning metadata currently uses a nullable full thinking-level map: `null` means reasoning support is unknown and Core should not try reasoning, while a non-null map records which canonical Model Thinking Levels are available and `null` entries for unavailable levels. During the AI SDK migration, the existing provider-value strings are treated as temporary availability markers and provider-family mappings in Core choose the concrete AI SDK options. An effective `off` thinking level means the user/config explicitly disabled thinking for that Agent Run, while runtime `null` thinking means Core sends no provider thinking options. The provider-value map should be redesigned after the AI SDK migration into a provider-family-aware capability/config shape.
