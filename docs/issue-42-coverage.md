...
| 14. Job orchestration discussion | Execution, Action, Agent Run; Execution Policy | Partial / Superseded | `docs/core/CONTEXT.md`; `docs/core/adr/2026-06-07-1150-versioned-project-execution-policy.md`; `docs/core/model.ts` | Domain hierarchy is documented. Scheduler claim/lock/heartbeat/retry/cancellation details still need implementation spec. |
| 15. WorkRun discussion | Execution | Covered / Superseded | `docs/core/CONTEXT.md`; `docs/core/model.ts`; `docs/core/api.ts` | Original `WorkRun` term is superseded. |
| 16. Pause discussion | Decision; execution paused for Decision event | Partial / Superseded | `docs/core/CONTEXT.md`; `docs/core/model.ts`; `docs/core/api.ts` | Pause is represented through Decisions in the API sketch, but no standalone Pause term or detailed pause/resume behavior spec exists. |
...
| 21. Configuration discussion | Project/Repository config, Execution Policy, Secrets, Secret Bindings | Partial | `docs/core/model.ts`; `docs/core/api.ts`; relevant Core ADRs | Stable model/config pattern exists in sketches. Full layered config resolution semantics are not yet a standalone spec. |
...
| 7 | Project Type modeling | Project Type, Source Control Project, Project config | Covered | `docs/core/CONTEXT.md`; `docs/core/model.ts`; `docs/core/adr/2026-06-07-0915-github-only-source-control-projects.md` | v1 supports GitHub Source Control Projects only, while keeping Project Type language broader. |
...
| 11 | Execution model | Execution, Action, Agent Run | Covered | `docs/core/CONTEXT.md`; `docs/core/model.ts`; `docs/core/api.ts` | Execution groups Actions/Agent Runs but does not own lifecycle state. |
...
| 13 | Workflow Project Type V2 | Future Project Type possibility | Deferred / Partial | `docs/core/CONTEXT.md` | `Project Type` glossary mentions workflow generically, but Workflow Project Type remains a V2 candidate and has no ADR/spec. This is intentional unless/until v2 planning starts. |
...
12. **Workflow Project Type V2 note/spec** — only if/when v2 workflow orchestration becomes active work.
