...
| 16. Pause discussion | Decision | Partial / Deferred | `docs/core/CONTEXT.md` | Decision remains glossary-level only; pause/resume behavior and Decision-raising actions are not specified in v1 sketches yet. |
| 17. Decisions discussion | Decision | Partial / Deferred | `docs/core/CONTEXT.md` | Decision remains glossary-level only. Exact model shape, payloads, expiration, and who may answer decisions need PRD/spec. |
...
| 12 | Agent Run model | Agent Run, planning Agent Run, execution Agent Run | Covered | `docs/core/CONTEXT.md`; `docs/core/model.ts`; `docs/core/api.ts` | Agent Run is not repository-specific and does not own authoritative state. Decision remains glossary-level only. |
