...
**Server Consumer Sandbox Provider**:
The Server Consumer's deployment-specific raw implementation of Core's `consumer-managed` Sandbox Provider. In v1 it runs saved `consumer-managed` OCI images locally with microsandbox, while Core only sees the Consumer-managed provider boundary and does not know whether the implementation is local, remote, or microVM-backed.
_Avoid_: Core-owned Vercel provider, Agent Run Profile, Vercel runtime
