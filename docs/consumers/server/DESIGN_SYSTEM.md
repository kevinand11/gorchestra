...
- **Full-width structure**: borders and row dividers should run edge-to-edge within their owning rail, page, or section. Do not wrap content in padded parent boxes that inset the separator line.
...
- Page headers may have horizontal padding; section dividers below them should span the full content width.
...
- Use a shared `UiCallout` component for flat left-rule guidance, warning, notice, or error copy instead of page-local callout markup.
