<template>
	<NuxtLayout name="portfolio">
		...
		<section class="border-b border-dimmer px-3 py-3">
			<UiForm class="max-w-[720px]" @submit.prevent="createProvider()">
				...
				<section class="border-t border-dimmer pt-3">
					...
					<div v-if="providerForm.headers.length === 0" class="mt-3 border-l-2 border-dimmer py-1 pl-3 text-sz-helper text-dim">
					...
					<div v-else class="mt-3 grid gap-3">
						<div
							...
							:key="index"
							class="grid gap-3 border-b border-dimmer pb-3 md:grid-cols-[1fr_1fr_auto]">
					...
					<p v-if="providerForm.errors.headers" class="m-0 mt-2 text-sz-helper text-error">{{ providerForm.errors.headers }}</p>
				</section>
