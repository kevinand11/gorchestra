			...
			<div v-else-if="provider" class="grid gap-0">
				...
				<section>
					...
					<div v-if="provider.models.length === 0" class="border-b border-dimmer px-3 py-4">
						<UiCallout>Add a Model from the right rail before selecting this provider in Portfolio Config.</UiCallout>
					</div>
					<div v-else>
						<div
		...
		<template v-if="provider" #right>
			<aside class="grid gap-5 p-3">
				<section class="grid gap-3">
					...
					<UiForm class="grid gap-3" @submit.prevent="saveProvider()">
				...
				<section class="grid gap-3 border-t border-dimmer pt-4">
					<div>
						<h2 class="m-0 text-sz-subsection font-semibold">Model actions</h2>
						<p class="m-0 mt-1 text-sz-helper leading-5 text-dim">
							Add, rename, preflight, archive, or unarchive Models under this Provider.
						</p>
					</div>
					<UiForm class="grid gap-3" @submit.prevent="createModel()">
					...
					<div class="border-t border-dimmer pt-3">
						...
						<div v-if="selectedModel" class="mt-3 grid gap-3">
							<UiForm class="grid gap-3" @submit.prevent="saveModel()">
								<UiFormGroup label="Model name" for-id="selected-model-name" :error="modelUpdateForm.errors.name">
									<UiInput
										id="selected-model-name"
										v-model="modelUpdateForm.name"
										:invalid="!!modelUpdateForm.errors.name" />
								</UiFormGroup>
								<UiButton
									type="submit"
									variant="secondary"
...
watch(
	...
	(model) => {
		...
		modelUpdateForm.loadEntity({ name: model.name })
