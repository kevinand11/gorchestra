...
- Portfolio-scoped Project UI/API routes use explicit Core command/query boundaries; add explicit Core operations before adding standalone Core routes for additional Portfolio facts.
...
- Nuxt client navigation uses `/sign-in` for Email OTP authentication, `/select` for Workspace/Portfolio selection and first Workspace Provisioning, `/projects` for selected-Portfolio Project state, and `/` as an auth/selection redirect landing route.
