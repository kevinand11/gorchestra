...
- Nuxt client navigation uses `/sign-in` for Email OTP authentication, `/select` for Workspace/Portfolio selection and first Workspace Provisioning, `/projects` for selected-Portfolio Project state, `/projects/:projectId` for Project detail and Repository setup, `/projects/:projectId/repositories/new` and `/projects/:projectId/repositories/:repositoryId` for Repository creation/detail, `/secrets` for selected-Portfolio Secret state, and `/` as an auth/selection redirect landing route. Selected-Portfolio pages use a shared selected-Portfolio nav with NuxtLink active classes rather than extracted class-name variables.
