...
- `src/commands/run-delivery-work/artifact-validation.ts` owns command-local validation selection and write-transaction dispatch around the claimed Delivery Context; Artifact creation provider calls and result writes belong in the `*-needs-artifact-creation` handlers.
