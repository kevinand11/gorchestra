...
- `src/utils/` owns internal shared utilities, including `utils/types.ts` for shared public primitive types such as `Result`, `utils/storage.ts` for generic validated Core storage/runtime helpers, and `utils/work-state/` for derived Delivery/Slice work-state logic split by Delivery, Slice, shared facts, and shared helpers.
