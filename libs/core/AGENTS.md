...
- `src/core.ts`, `src/services.ts`, `src/errors.ts`, and `src/validation.ts` own the remaining split Core API concerns, Core runtime opening/preflight, Core Services/runtime boundary direction, validation, and error contracts.
- `src/utils/` owns internal shared utilities, including `utils/types.ts` for shared public primitive types such as `Result`, `utils/storage.ts` for generic validated Core storage/runtime helpers, `utils/delivery-preflight.ts` for shared minimal Delivery preflight resolution, and `utils/work-state/` for derived Delivery/Slice work-state logic split by Delivery, Slice, shared facts, and shared helpers.
