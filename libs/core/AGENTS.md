...
- Keep query files focused on one `Queries.Core` method each and use object inputs rather than positional arguments for public query calls. Project-scoped Plan and Delivery reads validate the Project boundary before returning data; Delivery read models may expand target Repository and Slice data needed by consumers while preserving stored Delivery lifecycle fields.
