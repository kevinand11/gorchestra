...
- `src/commands/run-delivery-work/preflight.ts` owns command-local scheduler preflight orchestration around Delivery Context reads, provider-backed checks, stale-write readiness, and failed `validate-preflight` Action writes; keep generic readiness primitives in `src/utils/delivery-preflight.ts`.
- `src/commands/run-delivery-work/artifact-creation.ts` and `src/commands/run-delivery-work/artifact-validation.ts` own command-local Artifact creation/validation claim orchestration, provider-call or write-transaction dispatch, and stale-write recording around the state-specific handlers.
- Use `withTwoPhaseTransaction` from `src/utils/storage.ts` for command flows that intentionally split read-claim, outside-transaction work, and freshness-checked write phases.
