...
- `src/commands/run-delivery-work/artifact-creation.ts` and `src/commands/run-delivery-work/artifact-validation.ts` own command-local Artifact creation/validation claim orchestration, provider-call or write-transaction dispatch, and result recording around the claimed Delivery Context.
