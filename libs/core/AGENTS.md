...
- Model Provider Protocol operation call sites should use the family façade on `runtime.providers.modelProviderProtocols`; protocol dispatch, Model Provider Secret value resolution, AI SDK language-model resolution, and protocol-named readiness summaries belong in `src/providers/model-provider-protocol/`, while Agent Run runtime owns AI SDK `streamText` turn execution, tool wrapping, live deltas, and transcript event persistence.
