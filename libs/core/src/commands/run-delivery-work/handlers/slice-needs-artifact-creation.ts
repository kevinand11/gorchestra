...
export type SliceArtifactCreationClaim = SourceControlCreateSliceArtifactInput & {
	deliveryId: string
	sliceId: string
}

export function sliceArtifactCreationClaim(
	context: Pick<ResolvedDeliveryHandlerContext, 'deliveryContext' | 'workResolution'>,
): CoreResult<SliceArtifactCreationClaim | null, RunDeliveryWorkHandlerResult extends CoreResult<unknown, infer TError> ? TError : never> {
...
export async function recordSliceArtifactCreationResult(
	...
	claim: SliceArtifactCreationClaim,
	creation: SourceControlArtifactCreation,
): Promise<RunDeliveryWorkHandlerResult> {
	const current = sliceArtifactCreationStillCurrent(context, deliveryState, claim)
	if (!current.ok) return current
	if (!current.value) return noObservedArtifactCreationWrite()

	return creation.type === 'passed'
		? writePassedSliceArtifactCreation(context, claim)
		: writeFailedSliceArtifactCreation(context, claim.sliceId, creation.summary)
}
...
function claimForSlice(
	context: Pick<ResolvedDeliveryHandlerContext, 'deliveryContext'>,
	slice: Slice,
): CoreResult<SliceArtifactCreationClaim, InvariantViolationError> {
...
function sliceArtifactCreationStillCurrent(
	...
	if (deliveryState.type !== 'slices-incomplete' || !sliceArtifactClaimMatches(context, claim)) return { ok: true, value: false }
...
function sliceArtifactClaimMatches(context: ResolvedDeliveryHandlerContext, claim: SliceArtifactCreationClaim): boolean {
	return [
		context.deliveryContext.delivery.id === claim.deliveryId,
		context.deliveryContext.deliveryArtifact?.config.deliveryBranch === claim.sourceBranch,
	].every(Boolean)
}
...
async function writePassedSliceArtifactCreation(
	...
	const records = sliceArtifactCreationRecords(context, claim)
...
> {
	const artifact = sliceArtifactRecord(context, claim)
...
function sliceArtifactRecord(
	...
	return {
		...
		value: {
			id: id.value,
			sliceId: claim.sliceId,
			config: { type: 'source-control', sliceBranch: claim.sliceBranch },
...
if (import.meta.vitest) {
	...
	describe('Slice Artifact creation handler', () => {
		it('builds a deterministic provider claim from the Delivery Branch', async () => {
			const context = await handlerContext()

			expect(sliceArtifactCreationClaim(context)).toEqual({
				ok: true,
				value: {
		...
		it('stores a Slice Artifact and Action after provider success', async () => {
			const context = await handlerContext()
			const claim = sliceArtifactCreationClaim(context)
			if (!claim.ok || claim.value === null) throw new Error('Expected claim.')

			const result = await recordSliceArtifactCreationResult(context, { type: 'slices-incomplete' }, claim.value, {
				type: 'passed',
				mode: 'created',
				summary: 'created',
			})
