...
export function listRecordsByIds<Resource extends CoreIdResource>(
	resource: Resource,
	storage: CoreStorage,
	ids: Id[],
): Promise<Result<Array<CoreIdStorageRecord<Resource>>, StorageBoundaryError>> {
	...
	return listRecords(resource, storage, { where: (filter, fields) => filter.in(fields.id, ids) })
}
...
if (import.meta.vitest) {
	...
	describe('listRecords', () => {
		...
		it('lists records filtered by ID', async () => {
			const options = createTestCoreServices()
			const matching = {
				id: '01k00000000000000000000034',
				projectId: '01k00000000000000000000030',
			...
			const result = await listRecordsByIds('repository', options.storage, [matching.id])

			expect(result).toEqual({ ok: true, value: [matching] })
		})

		it('returns no records for empty IDs without reading storage', async () => {
			const options = createTestCoreServices()
			options.tx.repositories.fail.list = true

			const result = await listRecordsByIds('repository', options.storage, [])
